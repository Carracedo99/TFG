...
func _on_Rellenar_pressed():
	...
	if material_seleccionado == 1:
		if $Ventana/MaterialRellenar.get_selected_id() == 1:
			$Ventana/MaterialMezclar.add_item("Matraz erlenmeyer con vino", 1)
			$Ventana/MaterialMezclar2.add_item("Matraz erlenmeyer con vino", 1)
			
	if material_seleccionado == 3:
		if $Ventana/MaterialRellenar.get_selected_id() == 2:
			$Ventana/MaterialMezclar.add_item("Pipeta con a.sulfúciro 10%", 2)
			$Ventana/MaterialMezclar2.add_item("Pipeta con a.sulfúciro 10%", 2)

	if material_seleccionado == 3:
		if $Ventana/MaterialRellenar.get_selected_id() == 5:
			$Ventana/MaterialMezclar.add_item("Pipeta con almidón 1%", 3)
			$Ventana/MaterialMezclar2.add_item("Pipeta con almidón 1%", 3)
			
	if material_seleccionado == 6:
		if $Ventana/MaterialRellenar.get_selected_id() == 3:
			$Ventana/MaterialMezclar.add_item("Bureta con yodo 0,05N", 4)
			$Ventana/MaterialMezclar2.add_item("Bureta con yodo 0,05N", 4)


	if material_seleccionado == 3:
		if $Ventana/MaterialRellenar.get_selected_id() == 4:
			$Ventana/MaterialMezclar.add_item("Pipeta con NaOH 4N", 5)
			$Ventana/MaterialMezclar2.add_item("Pipeta con NaOH 4N", 5)

	if material_seleccionado == 5:
		if $Ventana/MaterialRellenar.get_selected_id() == 0:
			$Ventana/MaterialMezclar.add_item("Probeta con agua destilada", 6)
			$Ventana/MaterialMezclar2.add_item("Probeta con agua destilada", 6)



func _on_BotonComprobar_pressed():
	if Singleton.correcto8 == true:
		$BotonComprobar/Good.visible = true
		$BotonComprobar/Wrong.visible = false
		Singleton.bien8 = true
		Singleton.mal8 = false
	else:
		$BotonComprobar/Wrong.visible = true	#lo mismo pero con bien2 y mal2
		if Singleton.bien8 == false:
			Singleton.mal8 = true
...
#----------------------------------------------------------------------------
	...
	if $Ventana/MaterialMezclar.get_selected_id() == 17:
		if $Ventana/MaterialMezclar2.get_selected_id() == 2:
			#Singleton.correcto8 = true
			$Ventana/MaterialMezclar.add_item("Mezcla9", 18)
			$Ventana/MaterialMezclar2.add_item("Mezcla9", 18)

	if $Ventana/MaterialMezclar.get_selected_id() == 2:
		if $Ventana/MaterialMezclar2.get_selected_id() == 17:
			#Singleton.correcto8 = true
			$Ventana/MaterialMezclar.add_item("Mezcla10", 18)
			$Ventana/MaterialMezclar2.add_item("Mezcla10", 18)

#----------------------------------------------------------------------------

	if $Ventana/MaterialMezclar.get_selected_id() == 18:
		if $Ventana/MaterialMezclar2.get_selected_id() == 4:
			Singleton.correcto8 = true

	if $Ventana/MaterialMezclar.get_selected_id() == 4:
		if $Ventana/MaterialMezclar2.get_selected_id() == 18:
			Singleton.correcto8 = true
